...
class RTMPoseToAdaPose(nn.Module):
    ...
    def forward(
        ...
        # TODO tune
        max_px_scaling = 2.27 * 1.25

        # TODO These could be merged into one clean set to reduce nodes by half
        left_wrist_d, left_mcp_d = tch.norm(coco_main_metric_xyz[:, 9:10], dim=-1, keepdim=True), tch.norm(
            coco_main_metric_xyz[:, 17:18], dim=-1, keepdim=True)
        left_hand_metric_over_px = tch.abs((left_wrist_d - left_mcp_d) / (zd_prior[:, 9:10] - zd_prior[:, 100:101])).clamp_max(
            max_px_scaling)

        left_wrist_to_kp_euclidean_dist_in_px = zd_prior[:, 91:112] - zd_prior[:, 91:92]  # 21
        left_wrist_to_kp_euclidean_dist_in_metric = (
                                                            left_wrist_to_kp_euclidean_dist_in_px * left_hand_metric_over_px) + left_wrist_d
        ...
        right_wrist_d, right_mcp_d = tch.norm(coco_main_metric_xyz[:, 10:11], dim=-1, keepdim=True), tch.norm(
            coco_main_metric_xyz[:, 18:19], dim=-1, keepdim=True)
        right_hand_metric_over_px = tch.abs(
            (right_wrist_d - right_mcp_d) / (zd_prior[:, 10:11] - zd_prior[:, 121:122])).clamp_max(max_px_scaling)

        right_wrist_to_kp_euclidean_dist_in_px = zd_prior[:, 112:133] - zd_prior[:, 112:113]  # 21
        right_wrist_to_kp_euclidean_dist_in_metric = (
                                                             right_wrist_to_kp_euclidean_dist_in_px * right_hand_metric_over_px) + right_wrist_d
        ...
        coco_metric_xyz = tch.cat(
            [coco_main_metric_xyz[:, :17], l_foot_kp, r_foot_kp, face_kp, left_hand_metric_xyz, right_hand_metric_xyz],
            dim=1)
